...
class Pointnet2GMMPartSegHead(GMMSegHead):
    ...
    def __init__(self, 
        ...
        super(Pointnet2GMMPartSegHead,self).__init__(
                in_channels,
                channels,
                num_classes,
                decoder_params,
    ...
    def label_transform(self,inputs):
        inputs = rearrange(inputs, "b (h w) -> b 1 h w", h=(int)(math.sqrt(inputs.shape[1]*2))) 
        return inputs
    def base_feature_transform(self, inputs):
        ...
        for idx in range(len(inputs)):
            x = inputs[idx]
            sqrt_num = (int)(math.sqrt(x.shape[2]))
            if sqrt_num*sqrt_num == x.shape[2]:
                if sqrt_num ==1:
                    x = inputs[idx].repeat(1,1,2048)
                    inputs[idx] = rearrange(x, "b c (h w) -> b c h w", h=(int)(math.sqrt(x.shape[2]*2))) 
                else:
                    inputs[idx] = rearrange(x, "b c (h w) -> b c h w", h=(int)(math.sqrt(x.shape[2]))) 
            else:
                inputs[idx] = rearrange(x, "b c (h w) -> b c h w", h=(int)(math.sqrt(x.shape[2]*2))) 

        visul = inputs[0].shape[2:]
        outs = []
        for idx in range(len(inputs)):
            x = inputs[idx]
            conv = self.convs[idx]

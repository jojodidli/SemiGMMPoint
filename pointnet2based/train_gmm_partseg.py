...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='gmmseg_pointnet2_part_msg', help='model name')
...
def main(args):
        ...
        try:
            ...
            for k in list(base_ckpt.keys()):
                if k.startswith('MAE_encoder'):
                    ...
                    del base_ckpt[k]
            incompatible = classifier.load_state_dict(base_ckpt,strict=False)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            losses, seg_pred,base_feature = classifier(points, cls_label,target,eval=True) #
        ...
        with torch.no_grad():
            ...
            for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
                ...
                points = points.transpose(2, 1)
                cls_label = to_categorical(label, num_classes)
                losses, seg_pred,base_feature = classifier(points, cls_label,target,eval=True) #已知具体大类，分割出小类

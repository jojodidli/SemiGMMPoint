...
class PointNextEncoder(nn.Module):
    ...
    def forward_seg_feat(self, p0, f0=None):
        if hasattr(p0, 'keys'):
            p0, f0 = p0['pos'], p0.get('x', None) #([2, 24000, 3]), ([2, 4, 24000])
        if f0 is None:
            f0 = p0.clone().transpose(1, 2).contiguous()
        p, f = [p0], [f0] #0:([2, 24000, 3]), ([2, 4, 24000])
        for i in range(0, len(self.encoder)):
            _p, _f = self.encoder[i]([p[-1], f[-1]])# 1:([2, 24000, 3]),([2, 32, 24000])---2:([2, 6000, 3]),([2, 64, 6000])---3:([2, 1500, 3]),([2, 128, 1500])---4:[2, 375, 3],([2, 256, 375])---[2,93,3] [2,516,93]
            p.append(_p)
            f.append(_f)
        return p, f
...
class PointNextDecoder(nn.Module):
    def __init__(self,
        ...
        self.in_channels = encoder_channel_list[-1] #32
        skip_channels = encoder_channel_list[:-1]   #[32, 64, 128, 256, 512]
        ...
        self.out_channels = fp_channels[-n_decoder_stages] #32
    ...
    def forward(self, p, f):
        # f:[0,1,2,3,4,5] 
        # torch.Size([2, 4, 24000])
        # torch.Size([2, 32, 24000])   ->torch.Size([2, 32, 24000]) iter=4   --> decode forword返回值  最后将特征输入head得到torch.Size([2, 13, 24000])的logit
        # torch.Size([2, 64, 6000])    ->torch.Size([2, 64, 6000])  iter =3
        # torch.Size([2, 128, 1500])  ->torch.Size([2, 128, 1500]) iter=2
        # torch.Size([2, 256, 375])    -> torch.Size([2, 256, 375])  iter=1
        # torch.Size([2, 512, 93])

        # p:[0,1,2,3,4,5]
        # torch.Size([2, 24000, 3])
        # torch.Size([2, 24000, 3])
        # torch.Size([2, 6000, 3])
        # torch.Size([2, 1500, 3])
        # torch.Size([2, 375, 3])
        # torch.Size([2, 93, 3])
        for i in range(-1, -len(self.decoder) - 1, -1): #decode len=4
...
@MODELS.register_module()
class GMMPointNextDecoder(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
                 decoder_stages: int = 4, 
    ...
    def forward(self, p, f):
        # f:[0,1,2,3,4,5] 
        # 0:torch.Size([2, 4, 24000])
        # 1:torch.Size([2, 32, 24000])   ->torch.Size([2, 32, 24000]) iter=4   --> decode forword返回值  最后将特征输入head得到torch.Size([2, 13, 24000])的logit
        # 2:torch.Size([2, 64, 6000])    ->torch.Size([2, 64, 6000])  iter =3
        # 3:torch.Size([2, 128, 1500])  ->torch.Size([2, 128, 1500]) iter=2
        # 4:torch.Size([2, 256, 375])    -> torch.Size([2, 256, 375])  iter=1
        # 5:torch.Size([2, 512, 93])

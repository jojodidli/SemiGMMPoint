...
def voxelize(coord, voxel_size=0.05, hash_type='fnv', mode=0):
    ...
    idx_sort = np.argsort(key)# 返回排序的索引 key[idx_sort]就是排序数值
    key_sort = key[idx_sort]  #对key排序
    _, voxel_idx, count = np.unique(key_sort, return_counts=True, return_inverse=True) #count是unique（已经排序,提供idx_sort）的个数统计，unique[voxel_idx]可以得到原来的list
...
def crop_pc_pre(coord, feat, label, split='train',
            voxel_size=0.04, voxel_max=None,
            downsample=True, variable=True, shuffle=True):
    if voxel_size and downsample:
        # Is this shifting a must? I borrow it from Stratified Transformer and Point Transformer. 
    ...
    if voxel_max is not None:
        ...
        if N >= voxel_max:
            ...
            # crop_idx = np.argsort(
            #     np.sum(np.square(coord - coord[init_idx]), 1))[:voxel_max]
            init_idxx = np.arange(0,N)
            np.random.shuffle(init_idxx)
            crop_idx = init_idxx[:voxel_max]

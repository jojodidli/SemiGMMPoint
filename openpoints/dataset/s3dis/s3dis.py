...
from ..data_util import crop_pc, voxelize, crop_pc_pre
...
class S3DIS(Dataset):
    ...
    def __init__(self,
        ...
        if data_rate=="null":
            ...
            assert data_rate[0] == 'r'
            self.data_rate = "_"+ data_rate

        data_root = ""

        raw_root = os.path.join(data_root, 'stanford_indoor3d'+self.data_rate) # raw
        logging.info(raw_root)
        ...
        logging.info("data rate is : "+ self.data_rate)
...
class PreS3DIS(Dataset):
    ...
    def __init__(self,
        ...
        raw_root = os.path.join(data_root, 'stanford_indoor3d') # raw
        self.raw_root = raw_root
        data_list = sorted(os.listdir(raw_root))
        data_list = [item[:-4] for item in data_list if 'Area_' in item]
        if split == 'train':
    ...
    def __getitem__(self, idx):
        ...
        if self.presample:
            ...
            data_path = os.path.join(
                self.raw_root, self.data_list[data_idx] + '.pth')
            ...
            cdata[:, :3] -= np.min(cdata[:, :3], 0)
            coord, feat, label = cdata[:, :3], cdata[:, 3:6], cdata[:, 6:7]
            coord, feat, label = crop_pc_pre(
                coord, feat, label, self.split, self.voxel_size, self.voxel_max,
                downsample=not self.presample, variable=self.variable, shuffle=self.shuffle)
            num_pointpair = len(label)//3
            coord1 = np.concatenate((coord[:num_pointpair],coord[num_pointpair:num_pointpair*2]),axis=0)
            feat1 = np.concatenate((feat[:num_pointpair],feat[num_pointpair:num_pointpair*2]),axis=0)
            ...
            feat2 = np.concatenate((feat[:num_pointpair],feat[num_pointpair*2:num_pointpair*3]),axis=0)
        ...
        label1 = label1.squeeze(-1).astype(np.long)
        data1 = {'pos': coord1, 'x': feat1, 'y': label1}
        
        label2 = label2.squeeze(-1).astype(np.long)
        data2 = {'pos': coord2, 'x': feat2, 'y': label2}
        ...
        if 'heights' not in data1.keys():
            data1['heights'] =  torch.from_numpy(coord1[:, self.gravity_dim:self.gravity_dim+1].astype(np.float32))
        if 'heights' not in data2.keys():
            data2['heights'] =  torch.from_numpy(coord2[:, self.gravity_dim:self.gravity_dim+1].astype(np.float32))
